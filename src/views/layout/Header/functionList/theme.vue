...
const state = reactive({
  style: globalStore.theme.style,
  primaryColor: globalStore.theme.primaryColor,
  primaryTextColor: globalStore.theme.primaryTextColor,
  menuType: globalStore.theme.menuType
})
const themeColorArr = [
...
const options = reactive([
  { name: '显示logo', value:  globalStore.showLogo, store: 1 },
  { name: '显示面包屑导航', value: globalStore.showTabs, store: 2 },
  { name: '保持一个菜单展开', value: globalStore.expandOneMenu, store: 3 }
])
...
const change = (option) => {
  ...
  globalStore.changeOther(option.store)
}
setTheme()
</script>
